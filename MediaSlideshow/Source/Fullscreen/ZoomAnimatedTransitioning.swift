...
public class ZoomAnimatedTransitioningDelegate: NSObject, UIViewControllerTransitioningDelegate {
    ...
    /**
        ...
        - parameter slideshowView: MediaSlideshow instance to animate the transition from
        - parameter slideshowController: FullScreenViewController instance to animate the transition to
     */
    public init(slideshowView: MediaSlideshow, slideshowController: FullScreenSlideshowViewController) {
...
extension ZoomAnimatedTransitioningDelegate: UIGestureRecognizerDelegate {
    public func gestureRecognizerShouldBegin(_ gestureRecognizer: UIGestureRecognizer) -> Bool {
        ...
        if let currentSlide = referenceSlideshowController?.slideshow.currentSlide,
           let zoomable = currentSlide as? ZoomableMediaSlideshowSlide,
           zoomable.isZoomed() {
            return false
        }
...
class ZoomAnimator: NSObject {
    ...
    var referenceSlideshowView: MediaSlideshow?
    var parent: ZoomAnimatedTransitioningDelegate

    init(referenceSlideshowView: MediaSlideshow, parent: ZoomAnimatedTransitioningDelegate) {
        self.referenceSlideshowView = referenceSlideshowView
        self.referenceImageView = referenceSlideshowView.currentSlide?.transitionImageView()
        self.parent = parent
        super.init()
    }
...
class ZoomInAnimator: ZoomAnimator, UIViewControllerAnimatedTransitioning {
    ...
    func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        if let slide = toViewController.slideshow.currentSlide,
            let zoomable = slide as? ZoomableMediaSlideshowSlide,
        ...
        UIView.animate(withDuration: duration, delay: 0, usingSpringWithDamping: 0.7, initialSpringVelocity: 0, options: .curveLinear, animations: {
...
class ZoomOutAnimator: ZoomAnimator, UIViewControllerAnimatedTransitioning {
    ...
    private func animationParams(using transitionContext: UIViewControllerContextTransitioning) -> (TimeInterval, () -> Void, (Any) -> Void) {
        ...
        if let currentSlideshowItem = fromViewController.slideshow.currentSlide {
        ...
        if let referenceImageView = referenceImageView {
            ...
            if transitionImageView?.image != nil && referenceImageView.contentMode == .scaleAspectFit {
        ...
        let transitionView = UIImageView(image: transitionImageView?.image)
